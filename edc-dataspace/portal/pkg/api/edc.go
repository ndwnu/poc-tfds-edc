...
func (e *EdcAPI) GetRawDataConsumerPull(DataAddress DataAddress) (*RawData, error) {
	log.Println("[INFO] Starting GetRawDataConsumerPull method")

	// Log input details for debugging
	log.Printf("[DEBUG] DataAddress.Endpoint: %s", DataAddress.Endpoint)
	log.Printf("[DEBUG] DataAddress.Authorization: %s", DataAddress.Authorization)
	...
	resp, err := e.client.R().
		...
		Get(DataAddress.Endpoint)
	if err != nil {
		log.Printf("[ERROR] Error during HTTP request: %v", err)
		return nil, err
	}
	defer resp.RawBody().Close()
	log.Printf("[INFO] Response received, status code: %d, %s", resp.StatusCode(), resp.String())
	if resp.StatusCode() != 200 {
		return nil, fmt.Errorf("unable to get data from source: %s", resp.String())
	}
